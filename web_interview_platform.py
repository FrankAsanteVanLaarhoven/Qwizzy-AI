#!/usr/bin/env python3
"""
Web-Based Interview Intelligence Platform
Standalone web application with integrated AI-powered teleprompter
"""
import json
import requests
...
class WebInterviewPlatform:
    ...
    def setup_web_routes(self):
        ...
        @self.app.route('/')
        def index():
            return render_template('interview_platform.html')
        ...
        @self.app.route('/api/generate_qr')
        def generate_qr():
            try:
    ...
    def start_web_server(self):
        ...
        try:
            ...
            logger.info(f"Web interface opened at http://localhost:{self.web_port}")
        except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            print("\n🛑 Shutting down web interview platform...")
            self.is_listening = False
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎯 Interview Intelligence Platform</h1>
            <p>AI-Powered Real-Time Interview Assistance | Anika Bansal - AI/Web3 Frontend Engineer Role</p>
        </div>
    ...
    <script>
        ...
        async function generateQR() {
            try {
                ...
                if (data.qr_code) {
                    document.getElementById('qrCode').innerHTML = `<img src="${data.qr_code}" class="qr-code" alt="QR Code">`;
                    document.getElementById('qrUrl').innerHTML = `<p><strong>URL:</strong> ${data.url}</p>`;
...
"""
        ...
        with open(os.path.join(template_dir, "interview_platform.html"), "w") as f:
            f.write(html_content)

def main():
    print("🎯 Web-Based Interview Intelligence Platform")
    print("=" * 60)
    ...
    print("🌐 Platform: Web-Based Application (No GUI Dependencies)")
    print("=" * 60)
    print("🚀 Starting Web-Based Interview Intelligence Platform...")
    print("💡 A comprehensive web interface will open in your browser")
    print("🎤 Real-time audio listening and AI response generation")
    print("📱 Mobile interface with QR code generation")
    print("=" * 60)
    
    platform = WebInterviewPlatform()
    platform.run()
